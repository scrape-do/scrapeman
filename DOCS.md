...
- **Settings** — per-request proxy, timeout, redirect, **TLS certificate verification** (toggle off to bypass cert checks the way curl does with `-k` — useful for self-signed proxies, expired certs, mitmproxy debugging; the URL bar shows a red `Insecure` badge while the toggle is on; the global default for new requests lives under Settings → Network), HTTP version, **cookie jar toggle** (Postman-style "Send cookies" — when off the workspace cookie jar is bypassed for this request, no `Cookie` header on send and no `Set-Cookie` capture), and Scrape.do native mode. See [Proxy and Scrape.do Mode](#proxy-and-scrapedo-mode).
