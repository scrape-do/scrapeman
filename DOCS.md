...
- **Params** — key-value table with two-way URL sync. Editing a param updates the URL query string, and editing the URL bar updates the table. The table is the source of truth: it holds each value verbatim (including a value that is itself a URL with its own query, like `url=https://site.com/p?a=1&b=2`), keeps duplicate keys, and preserves row order. When such a value is folded into the URL its inner `&` is encoded as `%26` so it stays one row and reaches the server as one value; the cell still shows the readable `&`. Pasting a raw, unencoded nested URL straight into the URL bar is the one ambiguous case: it splits on every `&`, so build that value in the table instead.
...
- **Settings** — per-request proxy, timeout, redirect, **TLS certificate verification** (toggle off to bypass cert checks the way curl does with `-k` — useful for self-signed proxies, expired certs, mitmproxy debugging; the URL bar shows a red `Insecure` badge while the toggle is on; the global default for new requests lives under Settings → Network), HTTP version, **cookie jar toggle** (Postman-style "Send cookies" — when off the workspace cookie jar is bypassed for this request, no `Cookie` header on send and no `Set-Cookie` capture), **raw-body toggle** (`{{var}}` placeholders inside the body are normally substituted from the active environment; turn this on to send the body verbatim, e.g. when the receiving server expects literal `{{...}}` syntax — URL, headers, params, and auth still resolve), and Scrape.do native mode. See [Proxy and Scrape.do Mode](#proxy-and-scrapedo-mode).
