...
- **Modal dialogs flashed off-center before snapping to the middle.** The shared `slide-down-fade` entrance keyframe set `transform: translateY(...)`, which overrode each centered dialog's `translate(-50%, -50%)` while the animation ran, so the dialog painted off-center for a frame then jumped to center. Centered dialogs (the unsaved-changes confirm and every other one) now fade in, leaving their centering transform intact. Popovers and menus, which are popper-positioned, keep the slide.
...
- **First-launch welcome dialog** (M10 polish). Shown once per machine on the very first run. Three quick-start cards (open URL bar / pick a workspace / pick from recent workspaces), a six-shortcut crib sheet, and a `Read the docs` link. The `app:welcomeSeen` localStorage flag suppresses it on every subsequent launch; the existing post-update changelog dialog is independent and only fires on version bumps so the two never collide.
